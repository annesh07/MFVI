...
#'
#' @examples
#' mfvi2(D=2,N=100,T0=20,s1=0.001,s2=0.001,s01=0.001,s02=0.001,X=matrix(1,nrow=100, ncol=2),L20=0.001,Mu0=matrix(c(rep(0,2)), nrow=1),W1=0.001,W2=0.001,G1=0.001,G2=0.001,V1=matrix(0.0001, nrow = 20, ncol = 1),V2=matrix(0.0001, nrow = 20, ncol = 1),L1=matrix(0.0001, nrow = 20, ncol = 2),L2=matrix(0.0001, nrow = 20, ncol = 1),Plog=matrix(-3,nrow=100,ncol=20),maxit=1000)
mfvi2 <- function(D,N,T0,s1,s2,s01,s02,X,L20,Mu0,W1,W2,G1,G2,V1,V2,L1,L2,Plog,maxit){
  C00 <- diag(D)/L20
  Mu00 <- Mu0%*%solve(C00)
  source("C:/Users/ap15/Documents/MFVI/R/elbo2.R")

  f <- list()
  f[[1]] = ELBO2(D,N,T0,s1,s2,s01,s02,X,L20,Mu0,W1,W2,G1,G2,V1,V2,L1,L2,Plog)
  for (m in 1:maxit){
    for (j in 1:T0){
    ...
    f[[m+1]] = ELBO2(D,N,T0,s1,s2,s01,s02,X,L20,Mu0,W1,W2,G1,G2,V1,V2,L1,L2,Plog)
